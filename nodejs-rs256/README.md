# Backend for application listing (based on Auth0's quickstart Express Authorization for RS256-Signed Tokens)

Follow the instructions below to setup the configuration (.env file) and install and run the backend.
When the application is started a message in console is shown: "Backend listening on http://localhost:3010").
After this Node.js backend is started, start the React FE.
